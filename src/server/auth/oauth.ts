...
function validateRemoteUrl(label: string, raw: string, allowInsecure: boolean): URL {
  const url = new URL(raw);
  if (url.username || url.password || url.hash) {
    throw new Error(`${label} contains forbidden userinfo or fragment`);
  }
  if (url.protocol !== 'https:') {
    if (!(allowInsecure && url.protocol === 'http:' && isLoopbackHostname(url.hostname))) {
...
export function authorizationServerDiscoveryUrls(issuerRaw: string): URL[] {
  ...
  const urls: URL[] = [];
  if (path) {
    urls.push(new URL(`/.well-known/oauth-authorization-server${path}`, issuer.origin));
    urls.push(new URL(`/.well-known/openid-configuration${path}`, issuer.origin));
    urls.push(new URL(`${path}/.well-known/openid-configuration`, issuer.origin));
  } else {
    urls.push(new URL('/.well-known/oauth-authorization-server', issuer.origin));
    urls.push(new URL('/.well-known/openid-configuration', issuer.origin));
  }
  return urls;
}

async function fetchJson(url: URL, timeoutMs: number): Promise<Record<string, unknown>> {
...
function requireString(metadata: Record<string, unknown>, field: string): string {
  const value = metadata[field];
  if (typeof value !== 'string' || !value) throw new Error(`authorization metadata missing ${field}`);
  return value;
}

function optionalStringArray(metadata: Record<string, unknown>, field: string): string[] | undefined {
  const value = metadata[field];
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== 'string')) {
    throw new Error(`authorization metadata field ${field} must be a string array`);
  }
...
export async function discoverAuthorizationServer(
  ...
  for (const url of authorizationServerDiscoveryUrls(config.issuer)) {
    try {
      ...
      if (issuer !== configuredIssuer) {
        throw new Error(`issuer mismatch: expected ${configuredIssuer}, got ${issuer}`);
      }
      const authorizationEndpoint = requireString(raw, 'authorization_endpoint');
      const tokenEndpoint = requireString(raw, 'token_endpoint');
      validateRemoteUrl('authorization_endpoint', authorizationEndpoint, allowInsecure);
      validateRemoteUrl('token_endpoint', tokenEndpoint, allowInsecure);

      const pkceMethods = optionalStringArray(raw, 'code_challenge_methods_supported') ?? [];
      if (!pkceMethods.includes('S256')) {
        throw new Error('authorization server metadata must advertise PKCE S256');
      }

      if (config.clientRegistration === 'cimd' && raw.client_id_metadata_document_supported !== true) {
        throw new Error('clientRegistration=cimd requires client_id_metadata_document_supported=true');
      }
      if (config.clientRegistration === 'dcr') {
        const registrationEndpoint = requireString(raw, 'registration_endpoint');
        validateRemoteUrl('registration_endpoint', registrationEndpoint, allowInsecure);
      }

      const tokenAuthMethods = optionalStringArray(raw, 'token_endpoint_auth_methods_supported');
      if (tokenAuthMethods && !tokenAuthMethods.some((method) => method === 'none' || method === 'private_key_jwt')) {
        throw new Error('token endpoint must support ChatGPT-compatible client authentication (none or private_key_jwt)');
      }
      ...
      return {
        ...
        ...(typeof raw.client_id_metadata_document_supported === 'boolean'
          ...
          : {}),
        ...(tokenAuthMethods ? { token_endpoint_auth_methods_supported: tokenAuthMethods } : {}),
      };
    } catch (error) {
  ...
  throw new Error(`OAuth authorization-server discovery failed:\n  - ${failures.join('\n  - ')}`);
}

function resolveJwksUrl(config: OAuthHttpAuthConfig, metadata: AuthorizationServerMetadata): URL {
  const allowInsecure = Boolean(config.allowInsecureHttpForDevelopment);
  const raw = config.jwksUri ?? metadata.jwks_uri;
  if (!raw) throw new Error('authorization metadata missing jwks_uri and no oauth.jwksUri override was configured');
...
function parseScopes(payload: JWTPayload): string[] {
  ...
  if (typeof scp === 'string') {
    ...
    for (const scope of scp) if (typeof scope === 'string' && scope) scopes.add(scope);
  }
  return [...scopes];
}

function oauthPrincipalId(issuer: string, subject: string, clientId: string): string {
  const digest = createHash('sha256').update(`${issuer}\u0000${subject}\u0000${clientId}`).digest('hex').slice(0, 24);
  return `oauth:${digest}`;
}
...
export function buildProtectedResourceMetadata(config: OAuthHttpAuthConfig): Record<string, unknown> {
  return {
    ...
    ...(config.resourceDocumentation ? { resource_documentation: config.resourceDocumentation } : {}),
  };
}
...
export async function createOAuthRuntime(config: OAuthHttpAuthConfig): Promise<OAuthRuntime> {
  ...
  return {
    ...
    protectedResourceMetadataPaths: protectedResourceMetadataPaths(config.resource),
    resourceMetadataUrl: metadataUrl,
    async verifyAccessToken(token: string): Promise<VerifiedOAuthToken> {
      if (token.split('.').length !== 3) throw new Error('access token is not a JWT');
      ...
      if (tokenType && tokenType !== 'at+jwt' && tokenType !== 'application/at+jwt') {
      ...
      const subject = typeof payload.sub === 'string' && payload.sub ? payload.sub : 'anonymous-subject';
      const clientId =
