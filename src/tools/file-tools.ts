...
export function fileTools(): ToolDefinition[] {
  return [
    ...
    defineTool({
      name: 'file_move',
      description:
        'Move or rename a file or directory within the workspace. Both source and ' +
      ...
      handler: async (args, ctx) => {
        ...
        if (!existsSync(fromAbs)) return { ok: false, error: `Source does not exist: ${args.from}` };
    ...
    defineTool({
      name: 'file_copy',
      description:
        'Copy a file or directory within the workspace. Directories are copied ' +
    ...
    defineTool({
      name: 'list_directory',
      description:
        'List the entries of a directory within the workspace. Returns files and ' +
      ...
      handler: async (args, ctx) => {
        ...
        if (!existsSync(rootAbs)) return { ok: false, error: `Directory does not exist: ${target}` };
        if (!statSync(rootAbs).isDirectory()) return { ok: false, error: `Not a directory: ${target}` };
