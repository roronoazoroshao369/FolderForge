...
const PATTERNS: Array<{ tool: string; re: RegExp; map: (m: RegExpExecArray) => ParsedError }> = [
  {
    // TypeScript: src/x.ts(12,5): error TS2345: msg
    tool: 'typescript',
    re: /^(.+?)\((\d+),(\d+)\):\s+(error|warning)\s+TS\d+:\s+(.+)$/,
    map: (m) => ({ tool: 'typescript', file: m[1], line: +m[2]!, column: +m[3]!, severity: m[4] as 'error' | 'warning', message: m[5]! }),
  },
  {
    // ESLint: /path:12:5  error  msg  rule
    tool: 'eslint',
    re: /^\s*(\d+):(\d+)\s+(error|warning)\s+(.+?)(?:\s{2,}[\w-/]+)?$/,
    map: (m) => ({ tool: 'eslint', line: +m[1]!, column: +m[2]!, severity: m[3] as 'error' | 'warning', message: m[4]! }),
  },
  {
