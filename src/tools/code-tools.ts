...
async function routeToSerena(ctx: ToolContext, toolName: string, args: Record<string, unknown>) {
  if (!ctx.container.adapters.isEnabled('serena')) {
    return {
      ok: false,
      error:
        'Serena adapter is disabled. Enable adapters.serena in config to use semantic code tools. ' +
...
function codeTool(name: string, description: string, mutates: boolean, props: Record<string, unknown>): ToolDefinition {
  return defineTool({
    name,
    description,
    group: 'code',
