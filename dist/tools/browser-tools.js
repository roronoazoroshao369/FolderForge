...
function boundedFlowResult(result) {
    const content = result.content?.map((block) => block.kind === 'image'
...
async function runBrowserFlow(args, ctx) {
    ...
    for (let index = 0; index < steps.length; index += 1) {
        if (ctx.control?.signal?.aborted)
            return { ok: false, error: 'Browser flow cancelled.', data: { evidence } };
        const raw = steps[index];
        if (!isRecord(raw))
            return { ok: false, error: `Browser flow step ${index + 1} must be an object.`, data: { evidence } };
...
export function browserTools() {
    return [
        ...
        defineTool({
            ...
            handler: async (args, ctx) => {
                try {
                    return { ok: true, data: await ctx.container.browserEmulation.apply(args) };
                }
                catch (error) {
                    return { ok: false, error: `Browser emulation failed: ${String(error)}` };
                }
            },
        }),
        defineTool({
